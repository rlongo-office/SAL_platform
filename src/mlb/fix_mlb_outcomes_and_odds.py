...
# at top of fix_mlb_outcomes_and_odds.py, add:
...
def fetch_mlb_outcomes(target_date: str):
    """
    Pull Final games for target_date ±1 day, upsert into mlb_game_outcomes.
    Returns list of dicts with keys: game_id, away_id, home_id, start_time.
    """
    ...
    for delta in (-1, 0, 1):
        ...
        resp = sess.get(f"{API}/schedule", params={
            "sportId": 1,
            "date":    d.isoformat(),
    ...
    cur  = conn.cursor()
    for g in out_games:
        winner = "away" if g["away_runs"] > g["home_runs"] else "home"
...
def backfill_outcomes(conn, start_dt, end_dt):
    """
    Uses your existing outcomes loader to INSERT any schedule.game_id
    not yet in mlb_game_outcomes, by fetching all finals around each date.
    """
    cur = conn.cursor()
    # find schedule IDs missing in outcomes
    cur.execute("""
    ...
    # dedupe dates — one fetch per date
    missing_dates = sorted({sched_date for _, sched_date in missing})
