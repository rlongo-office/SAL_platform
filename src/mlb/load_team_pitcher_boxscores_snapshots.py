...
def pg_connect() -> psycopg2.extensions.connection:
    """Return a Postgres connection with search_path set."""
...
def load_pitcher_boxscores(conn: psycopg2.extensions.connection, session: requests.Session) -> None:
    """Populate `pitcher_boxscores` and ensure pitchers exist in `players`."""
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)

    # ─── NEW: Pull all valid team IDs up front ─────────────────────────────────
    cur.execute("SELECT id FROM teams")
    valid_team_ids = {r["id"] for r in cur.fetchall()}
    ...
    insert_pb = (
        """
        INSERT INTO pitcher_boxscores (
            game_id, player_id, side, sequence, at_bats, hits_allowed, bb_allowed,
            hbp_allowed, sf_allowed, total_bases, runs_allowed, date_played
        ) VALUES (
            %(game_id)s, %(player_id)s, %(side)s, %(seq)s, %(at_bats)s, %(hits)s,
            %(bb)s, %(hbp)s, %(sf)s, %(tb)s, %(runs)s, %(date_played)s
    ...
    for g in games:
        ...
        for side in ("away", "home"):
            ...
            # ─── NEW: skip any All-Star/unknown team ────────────────────────────
            ...
            for seq, pid in enumerate(plist, start=1):
                ...
                tb = max(0, singles + 2 * doubles + 3 * triples + 4 * homers)
                ...
                logger.info(
                    "[Stage 2] Inserting pitcher_boxscore: game=%s team=%s pid=%s side=%s seq=%s at_bats=%s hits=%s bb=%s hbp=%s sf=%s tb=%s runs=%s",
                    gid, tid, pid, side, seq, atb, hits, bb, hbp, sf, tb, runs
                )
                cur.execute(
                    insert_pb,
                    {
                ...
                if cur.rowcount:
                    logger.info("[Stage 2]   ✓ inserted rowcount=%d", cur.rowcount)
                rows_inserted += cur.rowcount

    conn.commit()
    logger.info("[Stage 2] Inserted %d new pitcher boxscore rows", rows_inserted)
...
def build_pitcher_snapshots(conn: psycopg2.extensions.connection) -> None:
    ...
    cur.execute(
        """
        SELECT player_id, date_played, at_bats, hits_allowed, bb_allowed,
               hbp_allowed, sf_allowed, total_bases
          FROM pitcher_boxscores
    ...
    insert_ps = (
        """
        INSERT INTO pitcher_stats_snapshots (
            player_id, snapshot_date, cum_ab, cum_h, cum_bb, cum_hbp, cum_sf,
            cum_tb, obp_allowed, slg_allowed
        ) VALUES (
            %(player_id)s, %(snapshot_date)s, %(cum_ab)s, %(cum_h)s, %(cum_bb)s,
            %(cum_hbp)s, %(cum_sf)s, %(cum_tb)s, %(obp)s, %(slg)s
    ...
    conn.commit()
    logger.info("[Stage 3] Inserted %d pitcher snapshot rows", snapshots)
